...
import EventHandler from 'mdb-ui-kit/src/js/mdb/dom/event-handler';
import Manipulator from 'mdb-ui-kit/src/js/mdb/dom/manipulator';
import SelectorEngine from 'mdb-ui-kit/src/js/mdb/dom/selector-engine';
import {getjQuery, getUID, typeCheckConfig} from 'mdb-ui-kit/src/js/mdb/util';
import Input from 'mdb-ui-kit/src/js/free/input';
import SelectOption from './select-option';
import SelectionModel from './selection-model';
import {DOWN_ARROW, END, ENTER, ESCAPE, HOME, UP_ARROW} from 'mdb-ui-kit/src/js/mdb/util/keycodes';
import allOptionsSelected from './util';
import {getDropdownTemplate, getOptionsListTemplate, getWrapperTemplate} from './templates';
...
class Select {
		...
		nodes.forEach((node) => {
			if (node.nodeName === 'OPTGROUP') {
				...
				groupOptions.forEach((option) => {
					if (option.nodeName === 'OPTION' && option.value !== '') {
						optionGroup.options.push(this._createOptionObject(option, optionGroup));
					}
				});
				options.push(optionGroup);
			} else if (node.nodeName === 'OPTION' && node.value !== '') {
	...
	_updateLabelPosition(forceClose = false) {
		...
		if (this._input.value !== '' || (this._isOpen && !forceClose)) {
	...
	_filterOptions(searchTerm) {
		...
		if (hasFilteredOptions) {
			...
			this.optionsWrapper.innerHTML = this._getNoResultTemplate();
		}
	}
	...
	close() {
		...
		setTimeout(() => {
			...
			this._updateLabelPosition(true);
		}, 0);
